...
class EasyMega_Menu_Item
{
	...
	function setup_post_query()
	{
		$default = array(
			...
			'order'             => '',
			'show_all_link'     => '',
			'all_item_text'     => esc_html__('All', 'easymega'),
			'all_item_link'     => '',
		);
	...
	function setup_menu_item($menu_item)
	{
		...
		if ($menu_item['type'] == 'custom' && $menu_item['menu_item_id'] == 'home') {
			if (! $menu_item['title']) {
				$menu_item['title'] = esc_html__('Home', 'easymega');
			}
			$menu_item['url'] = home_url('/');
		} elseif ($menu_item['type'] == 'custom') {

			if (! $menu_item['title']) {
				$menu_item['title'] = esc_html__('Untitled', 'easymega');
			}
			if (! $menu_item['url']) {
				$menu_item['url'] = '#';
			}
	...
	function render_builder_layout()
	{
		...
		if (! empty($this->item->mega_menu_layout)) {
			$child_li = EasyMega::get_theme_support('child_li');
			$tag = 'div';
			$tag_child = 'div';
			if ($child_li) {
				$tag = 'ul';
	...
	static function posts_content($query_args)
	{
		...
		if ($query->have_posts()) {
			ob_start();
			$file = EasyMega::get_template('nav-' . $post_type . '.php');
			if (! $file) {
				$file = EasyMega::get_template('nav-post.php');
			}

			$file = apply_filters('easymega_nav_post_tpl', $file, $post_type);
			if (is_file($file)) {
				include $file;
			}

			$paging_file = EasyMega::get_template('paging.php');
			if ($paging_file) {
				include $paging_file;
			}
	...
	static function get_post_query_args($data)
	{
		...
		if (isset($args['post__not_in'])) {
			if (empty($args['post__not_in'])) {
				...
				$args['post__not_in'] = explode(',', $args['post__not_in']); // phpcs:ignore WordPressVIPMinimum.Performance.WPQueryParams.PostNotIn_post__not_in	
				$args['post__not_in'] = array_map('trim', $args['post__not_in']); // phpcs:ignore WordPressVIPMinimum.Performance.WPQueryParams.PostNotIn_post__not_in	
				$args['post__not_in'] = array_map('absint', $args['post__not_in']); // phpcs:ignore WordPressVIPMinimum.Performance.WPQueryParams.PostNotIn_post__not_in	
				$args['post__not_in'] = array_filter($args['post__not_in']); // phpcs:ignore WordPressVIPMinimum.Performance.WPQueryParams.PostNotIn_post__not_in	
				if (empty($args['post__not_in'])) {
					unset($args['post__not_in']);
				}
			} else { // is array
	...
	function render_post()
	{
		...
		if ($categories) {
			...
			if (count($categories) > 1 && $this->mega_menu_post['show_all_link'] && $this->mega_menu_post['show_all_link'] != 'no') {
				...
				$query_args['tax_query'] = $this->mega_menu_post['tax']; // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_tax_query	
