...
window.easymega_live_preview = window.easymega_live_preview || {};
window.easymega_live_previewing = 0;
...
( function( api, $ ) {
    ...
    $(document).ready( function(){
        ...
        var $drop = $( '<div id="easymega-widget-modal-drop"></div>' );
        var $menuDrop = $( '<div id="easymega-menu-modal-drop"></div>' );
        $( '#customize-controls').append( $drop );
        $( '#customize-controls').append( $menuDrop );
        var colTpl = template({}, '#easymega-wp-col-tpl');
        ...
        availableWidgets.find( '#available-widgets').append( '<a class="easymega-close-widgets" href="#"><span class="dashicons dashicons-no-alt"></span></a>' );
        ...
        megaPanel.prototype = $.extend( true, megaPanel.prototype, {
            ...
            changeId: function( menu_id ){
                this.megaPanelObj.attr( 'id', 'easymega-id-' + menu_id );
                this.menuId = menu_id;
                this.control = api.Menus.getMenuItemControl( this.menuId );
            ...
            loadData: function( open ){
                ...
                var m = this;
                var id = this.menuId;
                $( '.easymega-wp-btn[menu-id="'+id+'"]' ).addClass( 'updating-message' );
                this.ajax( {
                    ...
                    $( '.easymega-wp-btn[menu-id="'+id+'"]' ).removeClass( 'updating-message' );
                    m.init();
                    m.sortableItems();
            ...
            active: function(){
                ...
                if ( this._ready == 'no' ) {
                    ...
                    $( '.easymega-wp').removeClass( 'active' );
            ...
            setPanelHeight: function( height ){
                ...
                var headerH = $( '.easymega-tabs', this.megaPanelObj ).outerHeight();
                if ( height > wh ) {
                    height = wh;
                }
                var h = ( height - headerH ) ;

                $( '.easymega-contents', this.megaPanelObj).css( { height: h, maxHeight: h, padding: 0 } );
            },
            ...
            livePreview: function(){
                try {
                    ...
                    if ( window.easymega_live_preview[ this.menuId ] ) {
            ...
            renderLayout: function( settingValue ){
                ...
                $.each( settingValue, function( rowIndex, rowData ) {
                    ...
                    $.each( rowData, function( colIndex, colData ){
                        var colTpl = template( colData.settings , '#easymega-wp-col-tpl');
            ...
            renderDefaultLayout: function(){
                ...
                for ( var i = 0; i < numCol; i++) {
                    $('.easymega-layout-area .row', megaEditor.megaPanelObj).eq(0).find('.row-inner').append(colTpl);
                }
            },
            ...
            updateData: function(){
                ...
                $document.trigger( 'easymega_data_changed', [ settingValue, megaEditor ] );
            ...
            settingActions: function(){
                ...
                if ( this.settingValue.mega_enable != 1 ) {
                    ...
                    $( '.easymega-tabs .setting-tab', megaEditor.megaPanelObj).addClass( 'display-none hide' );
                    $( '.easymega-content:not(.tab-settings)', megaEditor.settingForm).addClass( 'display-none hide' );
                }
                // When input changes
                megaEditor.megaPanelObj.on('keyup change data_change', 'select, input, textarea', function (e) {
                    ...
                    if ( data.mega_menu_settings.enable == 1 || data.mega_menu_settings.enable == '1' ) {
                        ...
                        $( '.easymega-tabs .setting-tab', megaEditor.megaPanelObj).removeClass( 'display-none hide' ).removeAttr( 'style' );
                        $( '.field:not(.enable-field)', megaEditor.settingForm).removeClass( 'display-none hide' ).removeAttr( 'style' );
                        $( '.easymega-content:not(.tab-settings)', megaEditor.settingForm).removeClass( 'display-none hide' ).removeAttr( 'style' ).removeAttr( 'style' );
                        $( '.easymega-tabs .setting-tab[data-tab="'+data.mega_menu_settings.menu_type+'"]', megaEditor.megaPanelObj).removeClass( 'display-none hide' ).removeAttr( 'style' );
                    ...
                    if ( data.mega_menu_settings.menu_type == 'layout' ) {
                        $( '.easymega-tabs li[data-tab="layout"]', megaEditor.megaPanelObj).removeClass( 'display-none hide' );
                        $( '.easymega-tabs li[data-tab="post"]', megaEditor.megaPanelObj).addClass( 'display-none hide' );
                    } else {
                        $( '.easymega-tabs li[data-tab="layout"]', megaEditor.megaPanelObj).addClass( 'display-none hide' );
                        $( '.easymega-tabs li[data-tab="post"]', megaEditor.megaPanelObj).removeClass( 'display-none hide' );
                    }
            ...
            sortableItems: function(){
                var megaEditor = this;
                // Sortable widgets
                $(".easymega-layout-area .sortable",megaEditor.megaPanelObj ).each( function(){
                    if ( ! $( this).hasClass( 'ui-sortable' ) ) {
                        $( this ).sortable({
                            connectWith: ".easymega-layout-area .sortable",
            ...
            ajax: function( data, successCb, errorCb ){
                data._nonce = megamenuSettings._nonce;
                $.ajax( {
                    url : ajaxurl,
                    data: data,
                    type: "POST",
            ...
            setup: function(){
                ...
                var dragElement = $( ".easymega-drag", megaEditor.megaPanelObj );
                dragElement.draggable({
                    ...
                    drag: function( event, ui ) {
                        var wh = $( window).height();
                        var headerH = $( '.easymega-tabs', megaEditor.megaPanelObj ).outerHeight();
                        var h = wh - ( ui.offset.top + headerH ) ;
                        if ( h <  headerH ) {
                            dragElement.css( top, -1 );
                        } else {
                            $( '.easymega-contents', megaEditor.megaPanelObj).css( { height: h, maxHeight: h, padding: 0 } );
                        }
                ...
                $( '.panel-live-view', megaEditor.megaPanelObj).on( 'click', function( e ){
                    e.preventDefault();
                    if ( ! window.easymega_live_preview[ megaEditor.menuId ] ) {
                        window.easymega_live_preview[ megaEditor.menuId ] = true;
                        window.easymega_live_previewing = 'menu-item-'+megaEditor.menuId;
                        $( this ).addClass( 'viewing' );
                    } else {
                        window.easymega_live_preview[ megaEditor.menuId ] = false;
                ...
                // Tabs action
                megaEditor.megaPanelObj.on('click', '.easymega-tabs li:not(.no-action)', function (e) {
                    e.preventDefault();
                    var li = $(this);
                    megaEditor.megaPanelObj.find('.easymega-tabs li').removeClass('active');
                    li.addClass('active');
                    var c = li.attr('data-tab') || '';
                    if (c) {
                        $('.easymega-contents .easymega-content', megaEditor.megaPanelObj ).removeClass('active');
                        $('.easymega-contents .easymega-content.tab-' + c, megaEditor.megaPanelObj ).addClass('active');
                    }
                });
                ...
                megaEditor.megaPanelObj.on('click', '.mega-col-widgets .add-menu', function (e) {
                    ...
                    availableMenuItems.addClass('easymega-menu-items');
                ...
                $document.on( 'click', '.add-new-menu-item', function(){
                    ...
                    availableMenuItems.removeClass('easymega-menu-items');
                } );
                ...
                megaEditor.megaPanelObj.on('click', '.easymega-layout-area .widget .widget-control-save', function (e) {
                    e.preventDefault();
                    // var widget = $(this).closest('.widget');
       ...
       /// ---------------------------------------------------------------------------------------------------------------------------------------
        ...
        $document.on('click', '#available-menu-items.easymega-menu-items .menu-item-tpl', function (e) {
            e.preventDefault();
        ...
        $drop.on('click', function (e) {
            e.preventDefault();
            availableWidgets.removeClass('easymega-widgets');
            if ( window.megaMenuChosingColumn ) {
                window.megaMenuChosingColumn.removeClass( 'active' );
                window.megaMenuChosingColumn = null;
            }
        ...
        $document.on( 'click', '.easymega-wp-btn', function( e ) {
            e.preventDefault();
        ...
        $document.on( "click", '.easymega-close-widgets', function( e ) {
            e.preventDefault();
            if ( availableWidgets.hasClass( 'easymega-widgets' ) ) {
                $drop.trigger( 'click' );
            }
        } );
        ...
        $document.on( "keydown", function( event ) {
           if ( event.which === 27 ) {
               ...
               if ( availableWidgets.hasClass( 'easymega-widgets' ) ) {
                   $drop.trigger( 'click' );
               } else if ( availableMenuItems.hasClass( 'easymega-menu-items' ) ) {
        ...
        */
        ...
        var setMenuCss = function(){
            ...
            var css = ' body .easymega-contents{ height: '+( ph-hH )+'px; }';
            ...
            if ( /^nav_menu\[/.test( setting.id ) && false !== setting() ) {
                ...
                if (control) {
                    ...
                    if ( typeof control._mega_settings === "undefined" || '' == control._mega_settings ) {
                        ...
                        var setting_tpl = template(settingValue, '#easymega-wp-settings');
    ...
    api.controlConstructor['nav_menu_item'].prototype.ready = function () {
        ...
        var megaButton = $( '<button type="button" menu-id="'+menu_id+'" class="button easymega-wp-btn">'+megamenuSettings.mega_settings_label+'</button>' );
        ...
        $( document).trigger( 'easymega_item_ready', [ control, menu_id ] );
